<template>
  <div class="space-y-6">
    ...
    <div class="bg-white p-4 sm:p-5 rounded-3xl border border-slate-200/80 shadow-2xs flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
      <div class="flex items-center gap-3">
        <div class="w-9 h-9 rounded-2xl bg-emerald-50 text-emerald-700 border border-emerald-200 flex items-center justify-center font-black text-lg shadow-2xs">
          📊
        </div>
        <div>
          <h1 class="text-lg md:text-xl font-black text-slate-900 tracking-tight">Miamala ya Kila Siku & Mfuko wa Siku</h1>
          <p class="text-[11px] text-slate-500 font-medium">Usimamizi wa mapokezi, maingizo ya mauzo, malipo ya wakulima, na gharama za uendeshaji.</p>
        </div>
      </div>
      ...
      <div class="flex items-center gap-2 flex-wrap md:flex-nowrap w-full md:w-auto justify-end">
        <!-- Date Selector -->
        <div class="flex items-center gap-1.5 bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-xl shadow-2xs hover:border-emerald-400 transition">
          <span class="text-slate-500 text-xs">📅</span>
          <input 
            v-model="selectedDate" 
            type="date" 
            @change="fetchData"
            class="bg-transparent text-slate-800 font-black text-xs focus:outline-none cursor-pointer"
          />
        </div>

        <!-- Quick Filter Preset Buttons -->
        <div class="flex items-center gap-1 bg-slate-100 p-1 rounded-xl text-[11px] font-bold text-slate-600">
          <button 
            @click="setToday" 
            class="px-2.5 py-1 rounded-lg transition cursor-pointer"
            :class="selectedDate === todayDateStr ? 'bg-emerald-600 text-white font-black shadow-2xs' : 'hover:bg-slate-200'"
          >
            Leo
          </button>
          <button 
            @click="setYesterday" 
            class="px-2.5 py-1 rounded-lg transition cursor-pointer"
            :class="selectedDate === yesterdayDateStr ? 'bg-emerald-600 text-white font-black shadow-2xs' : 'hover:bg-slate-200'"
          >
            Jana
          </button>
        </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-6 gap-3.5">
      ...
      <div class="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs relative overflow-hidden group hover:border-slate-300 transition-all">
        <div class="text-[10px] font-black uppercase tracking-wider text-slate-400">Chanzo cha Mfuko (Opening)</div>
        <div class="text-base md:text-lg font-black text-slate-900 mt-1 font-mono">
      ...
      <div class="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs relative overflow-hidden group hover:border-emerald-300 transition-all">
        <div class="text-[10px] font-black uppercase tracking-wider text-emerald-600">Maingizo ya Mauzo {{ isSelectedToday ? '(Leo)' : '' }}</div>
        <div class="text-base md:text-lg font-black text-emerald-700 mt-1 font-mono">
      ...
      <div class="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs relative overflow-hidden group hover:border-blue-300 transition-all">
        <div class="text-[10px] font-black uppercase tracking-wider text-blue-600">Malipo ya Wakulima</div>
        <div class="text-base md:text-lg font-black text-blue-700 mt-1 font-mono">
      ...
      <div class="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs relative overflow-hidden group hover:border-red-300 transition-all">
        <div class="text-[10px] font-black uppercase tracking-wider text-red-500">Matumizi ya Ofisi</div>
        <div class="text-base md:text-lg font-black text-red-600 mt-1 font-mono">
      ...
      <div 
        ...
        class="bg-emerald-50/90 p-4 rounded-2xl border border-emerald-300 shadow-xs relative overflow-hidden group hover:border-emerald-500 transition-all cursor-pointer"
      >
        <div class="flex items-center justify-between">
          <div class="text-[10px] font-black uppercase tracking-wider text-emerald-900">Faida Safi ya Leo</div>
        ...
        <div class="text-base md:text-lg font-black font-mono mt-1" :class="todayStandaloneProfit >= 0 ? 'text-emerald-800' : 'text-red-600'">
          Tsh {{ todayStandaloneProfit.toLocaleString() }}
        </div>
        <div class="text-[9.5px] text-emerald-700 mt-1 font-bold">
      ...
      <div 
        class="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs relative overflow-hidden group hover:border-teal-400 transition-all"
      >
        <div class="text-[10px] font-black uppercase tracking-wider text-teal-700">Salio la Kufunga (Cashbox)</div>
        <div class="text-base md:text-lg font-black text-slate-900 mt-1 font-mono">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
      ...
      <div class="lg:col-span-2 bg-white p-5 rounded-3xl border border-slate-200/80 shadow-xs space-y-3 animate-fadeIn">
        <div class="flex items-center justify-between">
          <div>
            <h3 class="text-sm font-extrabold text-slate-900 flex items-center gap-2">
              <span>📈 Mwenendo wa Faida Safi ya Siku (Standalone Profit Trend)</span>
            </h3>
            <p class="text-[11px] text-slate-500 font-medium">Grafu inapima faida iliyopatikana siku hiyo tu (bila ya jana). Gusa mstari kuona mchanganuo wa mapato na matumizi.</p>
          </div>
          <span class="text-[10px] bg-emerald-100 text-emerald-800 font-black px-2.5 py-1 rounded-lg">Standalone Daily Profit</span>
        </div>
      ...
      <div class="bg-white p-5 rounded-3xl border border-slate-200/80 shadow-xs space-y-3 flex flex-col justify-between animate-fadeIn">
        <div class="flex items-center justify-between">
          <div>
          ...
          <div class="flex items-center gap-1 bg-slate-100 p-1 rounded-xl text-[10px] font-extrabold">
            <button 
              @click="setRightChartMode('income')" 
              class="px-2.5 py-1 rounded-lg transition cursor-pointer"
              :class="rightChartMode === 'income' ? 'bg-emerald-700 text-white font-black shadow-2xs' : 'text-slate-600 hover:text-slate-900'"
            >
              🟢 Mapato
            </button>
            <button 
              ...
              class="px-2.5 py-1 rounded-lg transition cursor-pointer"
              :class="rightChartMode === 'expense' ? 'bg-red-700 text-white font-black shadow-2xs' : 'text-slate-600 hover:text-slate-900'"
            >
              🔴 Matumizi
            </button>
          </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-200/80 shadow-xs overflow-hidden">
      <div class="px-6 py-4 border-b border-slate-100 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-slate-50/50">
        <!-- Tabs Navigation -->
        <div class="flex items-center gap-2 bg-slate-200/70 p-1 rounded-2xl">
          <button 
            ...
            :class="activeTab === 'ledger' ? 'bg-white text-emerald-900 shadow-xs' : 'text-slate-600 hover:text-slate-900'"
          >
          ...
          <button 
            ...
            class="px-4 py-2 rounded-xl text-xs font-extrabold transition-all cursor-pointer"
            :class="activeTab === 'expenses' ? 'bg-white text-emerald-900 shadow-xs' : 'text-slate-600 hover:text-slate-900'"
          >
          ...
          <button 
            ...
            class="px-4 py-2 rounded-xl text-xs font-extrabold transition-all cursor-pointer"
            :class="activeTab === 'intake' ? 'bg-white text-emerald-900 shadow-xs' : 'text-slate-600 hover:text-slate-900'"
          >
        ...
        <div class="relative w-full md:w-72">
          ...
          <input 
            ...
            class="w-full pl-9 pr-3 py-2 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-800 placeholder-slate-400 focus:outline-none focus:border-emerald-500 transition"
          />
        </div>
      </div>
      ...
      <div v-if="activeTab === 'ledger'" class="space-y-0">
        ...
        <div class="px-6 py-2.5 bg-slate-100/60 border-b border-slate-200/80 flex items-center justify-between gap-2 flex-wrap">
          <div class="flex items-center gap-1.5 text-[11px] font-bold text-slate-600">
            <span class="mr-1 text-slate-500 font-semibold">🔍 Chuja Aina ya Muamala:</span>
            <button 
              @click="ledgerTypeFilter = 'ALL'" 
              class="px-3 py-1 rounded-lg transition cursor-pointer"
              :class="ledgerTypeFilter === 'ALL' ? 'bg-slate-900 text-white font-black shadow-2xs' : 'bg-white hover:bg-slate-200 text-slate-700 border border-slate-200'"
            >
              Wote (Combined)
            </button>
            <button 
              ...
              class="px-3 py-1 rounded-lg transition cursor-pointer"
              :class="ledgerTypeFilter === 'INFLOW' ? 'bg-emerald-700 text-white font-black shadow-2xs' : 'bg-white hover:bg-emerald-50 text-emerald-800 border border-emerald-200'"
            >
              🟢 Mauzo Tu
            </button>
            <button 
              ...
              class="px-3 py-1 rounded-lg transition cursor-pointer"
              :class="ledgerTypeFilter === 'FARMER_PAYOUT' ? 'bg-blue-700 text-white font-black shadow-2xs' : 'bg-white hover:bg-blue-50 text-blue-800 border border-blue-200'"
            >
              🔵 Wakulima Tu
            </button>
            <button 
              ...
              class="px-3 py-1 rounded-lg transition cursor-pointer"
              :class="ledgerTypeFilter === 'EXPENSE' ? 'bg-red-700 text-white font-black shadow-2xs' : 'bg-white hover:bg-red-50 text-red-800 border border-red-200'"
            >
          ...
          <div class="text-[11px] text-slate-500 font-bold">
            Miamala Inayoonekana: <strong class="font-mono text-slate-900">{{ filteredCombinedLedger.length }}</strong>
          </div>
        </div>
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-left text-xs border-collapse">
            <thead>
              <tr class="bg-slate-100/70 text-slate-700 font-extrabold border-b border-slate-200/80 uppercase text-[10px] tracking-wider">
              ...
              >
                ...
                <td class="py-3 px-4 whitespace-nowrap">
                  <span 
                    ...
                    :class="{
                      'bg-emerald-100 text-emerald-800 border border-emerald-300': item.type === 'INFLOW',
                      'bg-blue-100 text-blue-800 border border-blue-300': item.type === 'FARMER_PAYOUT',
                      'bg-red-100 text-red-800 border border-red-300': item.type === 'EXPENSE'
                    }"
                  >
                ...
                <td class="py-3 px-4">
                  <div class="font-extrabold text-slate-900">{{ item.category }}</div>
                  <div class="text-[10px] text-slate-400 font-mono">{{ item.reference || 'N/A' }}</div>
                </td>
                <td class="py-3 px-4 text-slate-600 font-medium">
                  {{ item.description }}
                </td>
                <td class="py-3 px-4 text-right font-black font-mono text-emerald-700">
                  {{ item.inflow > 0 ? ('Tsh ' + item.inflow.toLocaleString()) : '-' }}
                </td>
                <td class="py-3 px-4 text-right font-black font-mono" :class="item.type === 'FARMER_PAYOUT' ? 'text-blue-700' : 'text-red-600'">
                  {{ item.outflow > 0 ? ('Tsh ' + item.outflow.toLocaleString()) : '-' }}
                </td>
                <td class="py-3 px-4 text-center text-[11px] font-bold text-slate-600">
      ...
      <div v-if="activeTab === 'expenses'" class="space-y-4 p-6">
        <div class="flex items-center justify-between">
          <div>
            <h3 class="text-sm font-extrabold text-slate-900">Orodha ya Matumizi ya Ofisi Tarehe {{ selectedDate }}</h3>
            <p class="text-xs text-slate-500">Chakula cha wafanyakazi, umeme, mafuta ya generator, mishahara ya vibarua na matengenezo.</p>
          </div>
        ...
        <div class="border border-slate-200 rounded-2xl overflow-hidden shadow-2xs">
          <table class="w-full text-left text-xs border-collapse">
            <thead>
              <tr class="bg-slate-100/70 text-slate-700 font-extrabold border-b uppercase text-[10px]">
            ...
            <tbody class="divide-y divide-slate-100 font-semibold text-slate-700">
              ...
              <tr v-for="exp in filteredExpenses" :key="exp.id" class="hover:bg-slate-50 transition">
                ...
                <td class="py-3 px-4 text-slate-600">{{ exp.description || 'Matumizi ya kawaida' }}</td>
                <td class="py-3 px-4 text-right font-black text-red-600 font-mono">Tsh {{ parseFloat(exp.amount || 0).toLocaleString() }}</td>
                <td class="py-3 px-4 text-center text-slate-500 font-mono">{{ formatDate(exp.date_incurred) }}</td>
                <td class="py-3 px-4 text-center">
                  <button @click="deleteExpense(exp.id)" class="text-red-500 hover:text-red-700 font-bold p-1 cursor-pointer">🗑️ Futakazi</button>
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <div v-if="activeTab === 'intake'" class="overflow-x-auto">
        <table class="w-full text-left text-xs border-collapse">
          ...
          <tbody class="divide-y divide-slate-100 font-semibold text-slate-700">
            ...
            <tr v-for="(b, idx) in filteredBatches" :key="b.id" class="hover:bg-slate-50">
              <td class="py-3 px-4 text-slate-400 font-mono text-[11px]">{{ idx + 1 }}</td>
              <td class="py-3 px-4 font-mono font-black text-emerald-800">{{ b.batch_code }}</td>
              <td class="py-3 px-4 font-extrabold text-slate-900">{{ b.farmer_name }}</td>
              <td class="py-3 px-4 font-bold text-slate-800">{{ b.crop_type }}</td>
              <td class="py-3 px-4 text-right font-black text-slate-900 font-mono">{{ ((b.initial_weight || b.current_weight || 0) * 1000).toLocaleString() }} Kg</td>
              <td class="py-3 px-4 text-center font-bold" :class="parseFloat(b.moisture_percentage||0) <= 13 ? 'text-emerald-700' : 'text-amber-600'">{{ b.moisture_percentage || '12.5' }}%</td>
              <td class="py-3 px-4 text-center font-extrabold text-slate-700">{{ b.storage_location || 'Silo A1' }}</td>
              <td class="py-3 px-4 text-center text-slate-500 font-mono">{{ formatDate(b.created_at) }}</td>
              <td class="py-3 px-4 text-center">
                <button @click="printGRN(b)" class="px-2.5 py-1 bg-emerald-100 text-emerald-800 rounded-lg font-extrabold hover:bg-emerald-200 transition cursor-pointer">🖨️ GRN PDF</button>
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div class="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden border border-slate-200">
        ...
        <div class="p-6 space-y-4 text-xs font-semibold text-slate-700">
          <div>
            ...
            <select v-model="expenseForm.category_name" class="w-full p-2.5 border rounded-xl font-bold bg-slate-50 focus:bg-white">
          ...
          <div>
            ...
            <input v-model.number="expenseForm.amount" type="number" placeholder="e.g. 35000" class="w-full p-2.5 border rounded-xl font-bold text-sm text-slate-900 font-mono"/>
          </div>

          <div>
            <label class="block mb-1 font-bold">Tarehe ya Matumizi *</label>
            <input v-model="expenseForm.date_incurred" type="date" class="w-full p-2.5 border rounded-xl font-bold bg-slate-50"/>
          </div>
          ...
          <div class="flex justify-end gap-2 pt-2">
            <button @click="modals.expense = false" class="px-4 py-2 bg-slate-200 text-slate-700 font-bold rounded-xl cursor-pointer">Ghairi</button>
    ...
    <div v-if="modals.profitBreakdown" class="fixed inset-0 z-[100] bg-slate-900/80 backdrop-blur-xs flex items-center justify-center p-4">
      <div class="bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden border border-slate-200 animate-fadeIn">
        ...
        <div class="p-6 space-y-5 text-xs text-slate-800 font-sans max-h-[80vh] overflow-y-auto">
          <!-- Step 1: DYNAMIC Real Revenue (Services & Interest Earned Today) -->
          <div class="bg-teal-50/70 border border-teal-200 p-4 rounded-2xl space-y-2">
            <div class="font-black text-teal-900 uppercase tracking-wider text-[11px] flex items-center justify-between">
              <span>1. Ada za Huduma & Riba za Mikopo (Mapato ya Faida)</span>
              <span class="font-mono text-xs text-teal-800 font-black">+ Tsh {{ todaySystemFeeIncome.toLocaleString() }}</span>
            </div>

            <!-- Dynamic Services & Deductions Breakdown -->
            <div class="space-y-1.5 text-slate-600 font-semibold border-t border-teal-200/80 pt-2">
              ...
              >
                <span class="flex items-center gap-1.5">
                  ...
                  <span v-if="item.type === 'loan_principal'" class="text-[9.5px] bg-amber-100 text-amber-900 px-1.5 py-0.5 rounded font-extrabold uppercase">Rejesho la Cashbox (Sio Faida)</span>
                </span>
                <span class="font-mono font-black" :class="item.type === 'loan_principal' ? 'text-amber-800' : 'text-teal-950'">Tsh {{ item.amount.toLocaleString() }}</span>
              </div>
            </div>
          </div>

          <!-- Step 2: DYNAMIC Operational Expenses Incurred Today (Matumizi ya Ofisi Peakee) -->
          <div class="bg-red-50/70 border border-red-200 p-4 rounded-2xl space-y-2">
            <div class="font-black text-red-900 uppercase tracking-wider text-[11px] flex items-center justify-between">
              <span>2. Matumizi ya Ofisi Peakee (Office Operational Expenses)</span>
              <span class="font-mono text-xs text-red-700 font-black">- Tsh {{ todayExpenses.toLocaleString() }}</span>
            </div>

            <!-- Dynamic Expenses Breakdown -->
            <div class="space-y-1.5 text-slate-600 font-semibold border-t border-red-200/80 pt-2">
              ...
              >
                <span class="flex items-center gap-1.5">
                  ...
                  <span>{{ getCategoryEmoji(exp.category_name) }} <strong class="text-slate-900">{{ exp.category_name }}</strong> <span class="text-slate-500">({{ exp.description || 'Matumizi' }})</span>:</span>
                </span>
                <span class="font-mono font-black text-red-600">Tsh {{ parseFloat(exp.amount||0).toLocaleString() }}</span>
              </div>
            </div>
          </div>

          <!-- Step 3: Standalone Daily Net Operating Profit -->
          <div class="p-4 rounded-2xl border-2 flex items-center justify-between" :class="todayStandaloneProfit >= 0 ? 'bg-emerald-50 border-emerald-400 text-emerald-950' : 'bg-red-50 border-red-400 text-red-950'">
            <div>
              <div class="text-[11px] font-black uppercase tracking-wider">3. FAIDA SAFI YA SIKU HIYO TU (Standalone Net Profit)</div>
              <p class="text-[10px] text-slate-500 font-medium">Ada za Huduma + Riba minus Matumizi ya Ofisi (bila Mkopo Mama).</p>
            </div>
            <div class="text-xl font-black font-mono" :class="todayStandaloneProfit >= 0 ? 'text-emerald-700' : 'text-red-600'">
              Tsh {{ todayStandaloneProfit.toLocaleString() }}
            </div>
          </div>

          <!-- Step 4: Full Cash Movement Audit -->
          <div class="bg-slate-50 border border-slate-200 p-4 rounded-2xl space-y-2 text-[11px] font-semibold text-slate-700">
            <div class="font-black text-slate-900 uppercase">4. Mzunguko wa Mfuko wa Dratiba (Office Cashbox Audit):</div>
            ...
            <div v-if="todayLoanPrincipalRecovery > 0" class="flex justify-between text-amber-800 font-bold">
            ...
            <div class="flex justify-between text-slate-900 font-black border-t border-slate-300 pt-1.5 text-xs">
              <span>(=) SALIO LA KUFUNGA SIKU (Office Cashbox):</span>
              <span class="font-mono font-black text-teal-800">Tsh {{ closingCashBalance.toLocaleString() }}</span>
            </div>
            <p class="text-[10px] text-slate-500 italic pt-1 border-t border-slate-200">
    ...
    <div v-if="modals.cashbookPDF" class="fixed inset-0 z-[100] bg-slate-900/80 backdrop-blur-xs flex items-center justify-center p-4">
      <div class="bg-white w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden border border-slate-200 animate-fadeIn">
        ...
        <div id="printableArea" class="p-8 bg-white text-slate-900 font-sans space-y-6 max-h-[80vh] overflow-y-auto print:p-0 print:max-h-none">
          ...
          <div class="flex items-start justify-between border-b-2 border-slate-900 pb-5">
            <div class="flex items-center gap-3.5">
              ...
              <div>
                <div class="text-xl font-black tracking-tight text-slate-900 uppercase">GALANOKI MILLING & WAREHOUSE</div>
                <div class="text-xs text-slate-600 font-medium leading-relaxed mt-0.5">
            ...
            <div class="text-right">
              <div class="text-base font-black text-emerald-900 tracking-tight uppercase">DAILY CASHBOOK STATEMENT</div>
              <div class="text-[11px] text-slate-600 font-mono mt-1 space-y-0.5">
          ...
          <div class="grid grid-cols-5 gap-2 p-4 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-bold">
            <div class="bg-white p-2.5 rounded-xl border border-slate-200">
              <div class="text-[9px] text-slate-400 uppercase tracking-wider font-extrabold">1. Opening Float</div>
              <div class="text-xs font-black text-slate-900 font-mono mt-0.5">Tsh {{ openingCashBalance.toLocaleString() }}</div>
            ...
            <div class="bg-emerald-50/70 p-2.5 rounded-xl border border-emerald-200">
              <div class="text-[9px] text-emerald-700 uppercase tracking-wider font-extrabold">2. Gross Inflows</div>
              <div class="text-xs font-black text-emerald-800 font-mono mt-0.5">Tsh {{ todayInflows.toLocaleString() }}</div>
              <div class="text-[8px] text-emerald-600 mt-0.5">Mauzo ya Wanunuzi</div>
            </div>
            <div class="bg-blue-50/70 p-2.5 rounded-xl border border-blue-200">
              <div class="text-[9px] text-blue-700 uppercase tracking-wider font-extrabold">3. Farmer Payouts</div>
              <div class="text-xs font-black text-blue-800 font-mono mt-0.5">Tsh {{ todayFarmerPayouts.toLocaleString() }}</div>
              <div class="text-[8px] text-blue-600 mt-0.5">Malipo Wakulima</div>
            </div>
            <div class="bg-red-50/70 p-2.5 rounded-xl border border-red-200">
              <div class="text-[9px] text-red-700 uppercase tracking-wider font-extrabold">4. Office Expenses</div>
              <div class="text-xs font-black text-red-800 font-mono mt-0.5">Tsh {{ todayExpenses.toLocaleString() }}</div>
              <div class="text-[8px] text-red-600 mt-0.5">Matumizi Ofisi</div>
            </div>
          ...
          <div class="p-3 bg-gradient-to-r from-teal-50 to-emerald-50 border border-teal-200 rounded-2xl text-xs flex justify-between items-center font-bold text-teal-950">
            <div class="flex items-center gap-2">
              <span class="text-base">💡</span>
              <div>
                <span>Faida Safi ya Huduma & Mikopo (Standalone Operating Profit):</span>
                <p class="text-[10px] text-teal-700 font-medium">Ada za huduma zilizokusanywa minus Matumizi ya Ofisi tarehe {{ formatDate(selectedDate) }}.</p>
              </div>
            </div>
            <span class="font-mono text-sm font-black text-emerald-900 bg-white px-3 py-1 rounded-xl border border-emerald-300">
              Tsh {{ todayStandaloneProfit.toLocaleString() }}
            </span>
          </div>
          ...
          <div class="space-y-2">
            <div class="text-xs font-black text-slate-900 uppercase tracking-wider flex items-center justify-between border-b border-slate-300 pb-1">
              <span>1. Mapato ya Huduma & Rejesho la Mikopo (Service Income Collected):</span>
              <span class="font-mono text-emerald-800 font-extrabold">Total: Tsh {{ todaySystemFeeIncome.toLocaleString() }}</span>
            </div>
            <table class="w-full text-left text-xs border-collapse">
              <thead>
                <tr class="bg-slate-100 font-extrabold text-slate-800 text-[10px] uppercase">
                  <th class="py-2 px-3 border border-slate-200">#</th>
                  <th class="py-2 px-3 border border-slate-200">Aina ya Huduma / Makato</th>
                  <th class="py-2 px-3 border border-slate-200 text-right">Kiasi Kilichokusanywa (TZS)</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-slate-200 font-semibold text-slate-700">
                ...
                <tr v-for="(item, idx) in todayDynamicDeductions" :key="idx" class="hover:bg-slate-50">
                  <td class="py-2 px-3 border border-slate-200 font-mono text-[11px] text-slate-400">{{ idx + 1 }}</td>
                  <td class="py-2 px-3 border border-slate-200 font-bold text-slate-900">• {{ item.label }}</td>
                  <td class="py-2 px-3 border border-slate-200 text-right font-black text-emerald-800 font-mono">Tsh {{ item.amount.toLocaleString() }}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div class="space-y-2">
            <div class="text-xs font-black text-slate-900 uppercase tracking-wider flex items-center justify-between border-b border-slate-300 pb-1">
              ...
              <span class="font-mono text-red-700 font-extrabold">Total: Tsh {{ todayExpenses.toLocaleString() }}</span>
            </div>
            <table class="w-full text-left text-xs border-collapse">
              <thead>
                <tr class="bg-slate-100 font-extrabold text-slate-800 text-[10px] uppercase">
                  <th class="py-2 px-3 border border-slate-200">Kipengele</th>
                  <th class="py-2 px-3 border border-slate-200">Maelezo</th>
                  <th class="py-2 px-3 border border-slate-200 text-right">Kiasi (TZS)</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-slate-200 font-semibold text-slate-700">
                ...
                <tr v-for="exp in filteredExpenses" :key="exp.id" class="hover:bg-slate-50">
                  <td class="py-2 px-3 border border-slate-200 font-bold text-slate-900">{{ exp.category_name }}</td>
                  <td class="py-2 px-3 border border-slate-200 text-slate-600">{{ exp.description || '-' }}</td>
                  <td class="py-2 px-3 border border-slate-200 text-right font-black text-red-600 font-mono">Tsh {{ parseFloat(exp.amount || 0).toLocaleString() }}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div class="space-y-2">
            <div class="text-xs font-black text-slate-900 uppercase tracking-wider border-b border-slate-300 pb-1">
            ...
            <table class="w-full text-left text-xs border-collapse">
              ...
              <tbody class="divide-y divide-slate-200 font-semibold text-slate-700">
                ...
                <tr v-for="(item, idx) in combinedLedger" :key="idx" class="hover:bg-slate-50">
                  <td class="py-2 px-3 border border-slate-200 font-mono text-[10px] text-slate-500">{{ formatDate(item.date) }}</td>
                  <td class="py-2 px-3 border border-slate-200 font-bold">
                    <span v-if="item.type === 'INFLOW'" class="text-emerald-800 font-black">Mauzo ya Zao</span>
                    <span v-else-if="item.type === 'FARMER_PAYOUT'" class="text-blue-800 font-black">Malipo Mkulima</span>
                    <span v-else class="text-red-700 font-black">Matumizi Ofisi</span>
                  </td>
                  <td class="py-2 px-3 border border-slate-200 font-mono text-[10px] text-slate-600">{{ item.reference }}</td>
                  <td class="py-2 px-3 border border-slate-200 text-slate-700 text-[11px]">{{ item.description }}</td>
                  <td class="py-2 px-3 border border-slate-200 text-right font-black font-mono text-emerald-800">
                    {{ item.inflow > 0 ? ('Tsh ' + item.inflow.toLocaleString()) : '-' }}
                  </td>
                  <td class="py-2 px-3 border border-slate-200 text-right font-black font-mono" :class="item.type === 'FARMER_PAYOUT' ? 'text-blue-800' : 'text-red-700'">
                    {{ item.outflow > 0 ? ('Tsh ' + item.outflow.toLocaleString()) : '-' }}
                  </td>
                </tr>
              </tbody>
          ...
          <div class="pt-8 border-t-2 border-slate-300 grid grid-cols-2 gap-12 text-xs text-slate-700">
            <div>
              <div class="font-black text-slate-900 mb-8 uppercase text-[11px]">Sahihi ya Mweka Hazina (Cashier):</div>
              <div class="border-b border-slate-400 w-3/4 mb-1"></div>
              <div class="text-slate-500 font-semibold">Mweka Hazina (Galonoki MS)</div>
            </div>

            <div>
              <div class="font-black text-slate-900 mb-8 uppercase text-[11px]">Sahihi ya Meneja wa Ghala (Warehouse Manager):</div>
              <div class="border-b border-slate-400 w-3/4 mb-1"></div>
              <div class="text-slate-500 font-semibold">Warehouse Manager Approval</div>
            </div>
          </div>
        </div>
      </div>
