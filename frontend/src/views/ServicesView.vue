<template>
  <div class="space-y-6 animate-fadeIn">
    ...
    <div class="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200/80 dark:border-slate-700/80 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
      ...
      <div class="flex items-center gap-2 flex-wrap">
        ...
        >
          ...
          <span>{{ t('registerService', 'Sajili Huduma Mpya') }}</span>
        </button>
      </div>
    ...
    <Teleport to="body">
      <div v-if="showModal" class="fixed inset-0 z-[100] bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-4 overflow-hidden">
        <div class="bg-white dark:bg-slate-900 w-full max-w-md max-h-[85vh] rounded-3xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 animate-fadeIn flex flex-col my-auto">
    ...
    <Teleport to="body">
      <div v-if="showCropModal" class="fixed inset-0 z-[110] bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-4 overflow-hidden">
        <div class="bg-white dark:bg-slate-900 w-full max-w-sm max-h-[85vh] rounded-3xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 animate-fadeIn flex flex-col my-auto">
          <div class="px-6 py-4 border-b border-emerald-800 flex items-center justify-between bg-gradient-to-r from-emerald-900 to-teal-900 text-white shrink-0">
            <h3 class="text-base font-extrabold">🌱 Usimamizi wa Mazao</h3>
            <button @click="showCropModal = false" class="text-emerald-200 hover:text-white p-1 cursor-pointer">✕</button>
          </div>
          <div class="p-6 space-y-4 text-xs font-semibold text-slate-700 dark:text-slate-200 overflow-y-auto">
            <div>
            ...
            <div class="flex justify-end pt-2 border-t border-slate-100 dark:border-slate-800">
              <button @click="showCropModal = false" class="px-4 py-2 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-200 font-bold rounded-xl cursor-pointer">Funga</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <Teleport to="body">
      <div v-if="showUnitModal" class="fixed inset-0 z-[110] bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-4 overflow-hidden">
        <div class="bg-white dark:bg-slate-900 w-full max-w-lg max-h-[85vh] rounded-3xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 animate-fadeIn flex flex-col my-auto">
          <div class="px-6 py-4 border-b border-teal-800 flex items-center justify-between bg-gradient-to-r from-teal-900 to-emerald-900 text-white shrink-0">
          ...
          <div class="p-6 space-y-5 text-xs font-semibold text-slate-700 dark:text-slate-200 overflow-y-auto">
            ...
            <div class="flex justify-end pt-2 border-t border-slate-100 dark:border-slate-800">
              <button @click="showUnitModal = false" class="px-5 py-2 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 text-slate-800 dark:text-slate-100 font-extrabold rounded-xl transition cursor-pointer">Funga</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <Teleport to="body">
      <div v-if="toast.show" 
           class="fixed top-6 right-6 z-[120] max-w-md w-full p-4 rounded-2xl shadow-2xl backdrop-blur-xl border flex items-start gap-3.5 text-white transition-all transform animate-fadeIn duration-200"
    ...
    <Teleport to="body">
      <div v-if="confirmModal.show" class="fixed inset-0 z-[120] bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-4 overflow-hidden">
        <div class="bg-white dark:bg-slate-900 w-full max-w-sm rounded-3xl shadow-2xl overflow-hidden border border-slate-100 dark:border-slate-800 p-6 space-y-4 text-center animate-fadeIn my-auto">
