  ...
  <div class="space-y-6 animate-fadeIn">
    ...
    <div class="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div>
        <div class="flex items-center gap-2 text-xs font-semibold text-slate-400 mb-1">
          <span>Mwanzo</span>
          <span>/</span>
          <span class="text-emerald-700 font-bold">Mauzo & Invoices</span>
        </div>
      ...
      <div class="flex items-center gap-3">
        <button 
          ...
          class="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-2xl font-extrabold text-xs transition flex items-center gap-1.5"
        >
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div class="bg-white p-5 rounded-3xl border border-slate-200/80 shadow-2xs flex items-center justify-between">
        <div>
          ...
          <div class="text-3xl font-black text-emerald-700 mt-1">{{ paidInvoicesCount }}</div>
          <div class="text-[11px] text-slate-500 mt-0.5">Paid Invoices</div>
        </div>
      ...
      <div class="bg-white p-5 rounded-3xl border border-slate-200/80 shadow-2xs flex items-center justify-between">
        <div>
          ...
          <div class="text-[11px] text-amber-700 font-bold mt-0.5">Tsh {{ pendingAmount.toLocaleString() }}</div>
        </div>
      ...
      <div class="bg-white p-5 rounded-3xl border border-slate-200/80 shadow-2xs flex items-center justify-between">
        <div>
          ...
          <div class="text-[11px] text-slate-500 mt-0.5">Corporate Buyers</div>
        </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-200/80 shadow-2xs overflow-hidden">
      <div class="p-5 border-b border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-3">
        <h3 class="text-base font-extrabold text-slate-900">Orodha ya Invoices na Settlement Records</h3>
        <div class="flex items-center gap-2.5 w-full sm:w-auto">
          <input 
            v-model="searchQuery" 
            type="text" 
            placeholder="Tafuta Invoice # au Mnunuzi..." 
            class="px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold focus:bg-white"
          />
          <select v-model="statusFilter" class="px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold">
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left text-xs">
          ...
          <tbody class="divide-y divide-slate-100 font-semibold text-slate-700">
            ...
            <tr v-for="inv in filteredInvoices" :key="inv.id" class="hover:bg-slate-50/80 transition">
              <td class="p-4 font-mono font-bold text-indigo-700">{{ inv.invoice_number }}</td>
              <td class="p-4 font-extrabold text-slate-900">{{ inv.buyer ? inv.buyer.name : (inv.buyer_name || 'Buyer Corporate') }}</td>
              <td class="p-4"><span class="px-2 py-0.5 bg-emerald-50 text-emerald-800 rounded-md font-extrabold text-[10.5px]">{{ inv.crop_type || 'Mchele Grade A' }}</span></td>
              <td class="p-4 text-right font-mono font-bold">{{ parseFloat(inv.quantity_kg || 10000).toLocaleString() }} Kg</td>
              <td class="p-4 text-right font-mono text-slate-500">Tsh {{ parseFloat(inv.unit_price || 1200).toLocaleString() }}</td>
              <td class="p-4 text-right font-black text-emerald-700">Tsh {{ parseFloat(inv.total_amount || 0).toLocaleString() }}</td>
    ...
    <div v-if="showPDFModal" class="fixed inset-0 z-50 bg-slate-900/80 backdrop-blur-xs flex items-center justify-center p-4">
      <div class="bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden border border-slate-200 animate-fadeIn">
        ...
        <div id="printableDocumentArea" class="p-8 bg-white text-slate-900 font-sans space-y-6 max-h-[75vh] overflow-y-auto print:p-0 print:max-h-none">
          <!-- Letterhead Header -->
          <div class="flex items-start justify-between border-b-2 border-emerald-600 pb-4">
            <div>
              <div class="text-2xl font-black tracking-tight text-emerald-800">GALANOKI MILLING ERP</div>
            ...
            <div class="text-right">
              <div class="text-xl font-black text-slate-900 tracking-tight">TAX INVOICE</div>
          ...
          <div class="grid grid-cols-2 gap-4 text-xs">
            <div class="p-3 bg-slate-50 rounded-2xl border border-slate-200">
              <div class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Mteja / Mnunuzi (Bill To):</div>
              <div class="text-sm font-black text-slate-900 mt-1">{{ selectedInv.buyer ? selectedInv.buyer.name : (selectedInv.buyer_name || 'Bakhresa Grain Milling Corp') }}</div>
              <div class="text-slate-600 font-semibold mt-0.5">Attn: {{ selectedInv.buyer ? selectedInv.buyer.contact_person : 'Procurement Dept' }}</div>
              <div class="text-slate-500 font-mono text-[11px]">Phone: {{ selectedInv.buyer ? selectedInv.buyer.phone : '+255 784 900 120' }}</div>
            </div>

            <div class="p-3 bg-emerald-50/60 rounded-2xl border border-emerald-200 flex items-center justify-between">
              <div>
                <div class="text-[10px] font-bold text-emerald-800 uppercase tracking-wider">Hali ya Malipo:</div>
                <div class="text-base font-black text-emerald-900 mt-0.5">{{ selectedInv.status === 'paid' ? 'PAID & SETTLED' : 'PAYMENT PENDING' }}</div>
                <div class="text-[10.5px] text-emerald-700 font-medium">Verified by Galanoki Finance</div>
              </div>
          ...
          <table class="w-full text-left text-xs border-collapse">
            ...
            <tbody class="divide-y divide-slate-200 text-slate-800 font-semibold">
              <tr>
                <td class="py-3">
                  <div class="font-bold text-slate-900">{{ selectedInv.crop_type || 'Mchele Grade A (Rice - Polished)' }}</div>
                  <div class="text-[10.5px] text-slate-500">Milling & Processing Certificate Verified</div>
                </td>
                ...
                <td class="py-3 text-right font-mono font-black text-slate-900">Tsh {{ (parseFloat(selectedInv.quantity_kg || 10000) * parseFloat(selectedInv.unit_price || 1200)).toLocaleString() }}</td>
              </tr>
            </tbody>
          </table>
          ...
          <div class="flex justify-end pt-2">
            <div class="w-64 space-y-2 text-xs font-semibold text-slate-700">
              ...
              <div class="flex justify-between text-sm font-black text-emerald-800 border-t border-slate-900 pt-2">
          ...
          <div class="pt-8 border-t border-slate-200 grid grid-cols-2 gap-8 text-[11px] text-slate-500">
            <div>
              <div class="font-bold text-slate-800 mb-6">Sahihi ya Meneja wa Kinu:</div>
              <div class="border-b border-slate-300 w-3/4 mb-1"></div>
              <div>Authorized Signatory & Stamp</div>
            </div>

            <div class="text-right flex flex-col items-end">
              <div class="w-20 h-20 bg-slate-100 border border-slate-300 rounded-xl p-1 flex items-center justify-center text-[9px] font-mono text-center text-slate-600 mb-1">
        ...
        <div class="p-4 bg-slate-50 border-t border-slate-200 flex justify-end gap-2">
          <button @click="showPDFModal = false" class="px-4 py-2 bg-slate-200 text-slate-700 font-bold rounded-xl text-xs">Funga</button>
    ...
    <div v-if="showNewInvoiceModal" class="fixed inset-0 z-50 bg-slate-900/70 backdrop-blur-xs flex items-center justify-center p-4">
      <div class="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden border border-slate-200 animate-fadeIn">
        ...
        <div class="p-6 space-y-4 text-xs font-semibold text-slate-700">
          <div>
            <label class="block mb-1 font-bold text-slate-800">Mnunuzi (Buyer) *</label>
            <input v-model="newInvForm.buyer_name" type="text" placeholder="e.g. Bakhresa Grain Milling Ltd" class="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl font-bold" />
          </div>

          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block mb-1 font-bold text-slate-800">Aina ya Zao *</label>
              <select v-model="newInvForm.crop_type" class="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl font-bold">
            ...
            <div>
              <label class="block mb-1 font-bold text-slate-800">Kiasi (Kg) *</label>
              <input v-model.number="newInvForm.quantity_kg" type="number" placeholder="e.g. 10000" class="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl font-bold" />
            </div>
          </div>

          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block mb-1 font-bold text-slate-800">Bei Kwa Kg (Tsh) *</label>
              <input v-model.number="newInvForm.unit_price" type="number" placeholder="e.g. 1200" class="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl font-bold" />
            </div>

            <div>
              <label class="block mb-1 font-bold text-slate-800">Status *</label>
              <select v-model="newInvForm.status" class="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl font-bold">
          ...
          <div class="p-3 bg-slate-50 rounded-2xl border border-slate-200 space-y-1 font-mono text-[11px]">
            ...
            <div class="flex justify-between text-slate-500"><span>VAT (18%):</span><span>Tsh {{ Math.round(newInvForm.quantity_kg * newInvForm.unit_price * 0.18).toLocaleString() }}</span></div>
            <div class="flex justify-between font-black text-emerald-700 text-xs pt-1 border-t"><span>Jumla Kuu:</span><span>Tsh {{ Math.round(newInvForm.quantity_kg * newInvForm.unit_price * 1.18).toLocaleString() }}</span></div>
          </div>

          <div class="flex justify-end gap-2 pt-2 border-t border-slate-100">
            <button @click="showNewInvoiceModal = false" class="px-4 py-2 bg-slate-200 text-slate-700 font-bold rounded-xl">Ghairi</button>
