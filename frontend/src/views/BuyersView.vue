  ...
  <div class="space-y-6 text-slate-800 dark:text-slate-100">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
      <div class="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-2xs flex items-center gap-4">
        ...
        <div>
          <p class="text-[11px] font-extrabold text-slate-400 uppercase tracking-wider">Wanunuzi Hai</p>
          <h3 class="text-2xl font-black text-slate-900 dark:text-white mt-0.5">{{ buyersList.length }}</h3>
          <p class="text-[10.5px] text-slate-400 mt-0.5">Makampuni waliosajiliwa</p>
        </div>
      </div>
      ...
      <div class="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-2xs flex items-center gap-4">
        ...
        <div>
          <p class="text-[11px] font-extrabold text-slate-400 uppercase tracking-wider">Jumla ya Manunuzi</p>
          <h3 class="text-xl font-black text-slate-900 dark:text-white mt-0.5">TZS {{ formatCurrency(totalSpentAll) }}</h3>
          <p class="text-[10.5px] text-slate-400 mt-0.5">Thamani ya ankara zote</p>
        </div>
      </div>
      ...
      <div class="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-2xs flex items-center gap-4">
        ...
        <div>
          ...
          <h3 class="text-xl font-black text-amber-600 dark:text-amber-400 mt-0.5">TZS {{ formatCurrency(totalUnpaidAll) }}</h3>
          <p class="text-[10.5px] text-slate-400 mt-0.5">Mabaki ya manunuzi hayajamalizwa</p>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-2xs overflow-hidden">
      ...
      <div class="p-4 border-b border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3 bg-slate-50/50 dark:bg-slate-900/50">
        <div class="font-extrabold text-sm text-slate-900 dark:text-white flex items-center gap-2">
          <span>📋 Orodha ya Wanunuzi Waliosajiliwa</span>
        ...
        <table class="w-full text-left border-collapse text-xs">
          <thead>
            <tr class="bg-slate-50/80 dark:bg-slate-800/40 text-slate-500 dark:text-slate-400 font-extrabold border-b border-slate-100 dark:border-slate-800 uppercase tracking-wider text-[10.5px]">
              ...
              <th class="py-3.5 px-4">Mtu wa Mawasiliano</th>
              <th class="py-3.5 px-4">Simu / Email</th>
              <th class="py-3.5 px-4">TIN / Namba ya Kodi</th>
              <th class="py-3.5 px-4">Ankara</th>
              <th class="py-3.5 px-4">Jumla ya Manunuzi</th>
          ...
          <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60 font-medium">
            ...
            >
              ...
              <!-- Phone & Email -->
              <td class="py-3.5 px-4">
                <div class="font-bold text-slate-800 dark:text-slate-200">{{ buyer.phone || 'N/A' }}</div>
                <div class="text-[10px] text-emerald-600 dark:text-emerald-400 font-semibold">{{ buyer.email || '' }}</div>
              </td>

              <!-- TIN / Tax Number -->
              <td class="py-3.5 px-4">
                <span class="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 font-mono text-[11px] font-bold text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
                  {{ buyer.tax_number || 'N/A' }}
                </span>
              </td>

              <!-- Invoices Count -->
              <td class="py-3.5 px-4 font-extrabold text-slate-800 dark:text-slate-200">
                <span class="px-2 py-0.5 rounded-md bg-blue-50 dark:bg-blue-950/60 text-blue-600 dark:text-blue-400 font-black text-[11px]">
                  {{ buyer.invoices_count || 0 }} Ankara
                </span>
              </td>
      ...
      <div v-if="showHistoryModal" class="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
        <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl w-full max-w-2xl overflow-hidden animate-fadeIn">
          ...
          <div class="p-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/50">
            <div>
              <h3 class="font-black text-base text-slate-900 dark:text-white flex items-center gap-2">
                <span>📜 Historia ya Manunuzi</span>
                <span class="text-emerald-600 dark:text-emerald-400 font-bold">({{ selectedBuyerHistory?.name }})</span>
              </h3>
              <p class="text-[11px] text-slate-400 mt-0.5">Orodha ya ankara zote zilizowahi kutolewa kwa mnunuzi huyu.</p>
            </div>
            ...
            <table class="w-full text-left border-collapse text-xs">
              <thead>
                <tr class="bg-slate-50 dark:bg-slate-800/60 text-slate-500 dark:text-slate-400 font-extrabold border-b border-slate-100 dark:border-slate-800 uppercase text-[10.5px]">
                  ...
                  <th class="py-2.5 px-3">VAT (18%)</th>
                  <th class="py-2.5 px-3">Jumla Kuu</th>
              ...
              <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60 font-medium">
                ...
                <tr v-else v-for="inv in buyerInvoicesHistory" :key="inv.id" class="hover:bg-slate-50/60 dark:hover:bg-slate-800/30">
                  ...
                  <td class="py-2.5 px-3 font-bold text-slate-500">
                    TZS {{ formatCurrency(inv.vat_amount) }}
                  </td>
                  <td class="py-2.5 px-3 font-black text-emerald-600 dark:text-emerald-400">
...
const totalUnpaidAll = computed(() => {
  return buyersList.value.reduce((acc, b) => acc + parseFloat(b.unpaid_amount || 0), 0);
});
...
const fetchBuyers = async () => {
  ...
  try {
    ...
    if (res.ok) {
      buyersList.value = await res.json();
    } else {
      triggerToast('Imeshindwa kupakua orodha ya wanunuzi', 'error');
    }
  } catch (err) {
    console.error(err);
    triggerToast('Kosa la mtandao wakati wa kupakua wanunuzi', 'error');
  } finally {
    loading.value = false;
  }
};
